...
int ServerSocketImpl::connectSocket() {
    ...
    if (setsockopt(sockFd, SOL_SOCKET, SO_REUSEADDR | SO_REUSEPORT, &enable, sizeof(int)) < 0) {
        LOG_ERROR_WITH_ERRNO("setsockopt( | SO_REUSEPORT) failed");
        throw ConnectException("setsockopt(SO_REUSEADDR | SO_REUSEPORT) failed");
    }
...
int ServerSocketImpl::acceptNewClientSock() {
    int clientSock = 0;
    if ((clientSock = accept(serverSocket_, (struct sockaddr *) nullptr, nullptr)) < 0) {
        LOG_ERROR_WITH_ERRNO("error accept new client");
        throw ConnectException("error accept new client");
    }
