...
bool ProxyServer::CashImpl::isElementInCash(std::string request) {
    std::hash<std::string> hasher;
    unsigned long int hashHeading = hasher(request);
    pthread_mutex_lock(&mutex);
//    std::cout << "check cache\n";

    for (auto it = _listCash.begin(); it != _listCash.end(); it++) {

        unsigned long int itHash = hasher((*it)->getHeading());
//        if (itHash == hashHeading) {
        std::cout << "wtf1\n";
        std::cout << request;
        std::cout << (*it)->getHeading();
        std::cout << "wtf2\n";
        if (request == (*it)->getHeading()) {
            pthread_mutex_unlock(&mutex);
            return true;
        }
    }
